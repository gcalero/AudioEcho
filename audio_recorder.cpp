...
void AudioRecorder::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq) {
  ...
  dataBuf->size_ = dataBuf->cap_;  // device only calls us when it is really
                                   // full
  if (engine_) {
      engine_->EngineService(ENGINE_SERVICE_MSG_RECORDED_AUDIO_AVAILABLE, dataBuf);
  }
...
SLboolean AudioRecorder::Start(void) {
  if (!freeQueue_ || !recQueue_ || !devShadowQueue_) {
    qDebug() << "[DEMO-AUDIO] ====NULL poiter to Start(" <<freeQueue_ << ", " << recQueue_ << ", " << devShadowQueue_;
  ...
  SLASSERT(result);
  qDebug() << "[DEMO-AUDIO] AudioRecorder::Start";

  for (int i = 0; i < RECORD_DEVICE_KICKSTART_BUF_COUNT; i++) {
    sample_buf *buf = NULL;
    if (!freeQueue_->front(&buf)) {
      qDebug() << "[DEMO-AUDIO] =====OutOfFreeBuffers @startingRecording @ (" << i << ")";
      break;
    }
    freeQueue_->pop();
  ...
  qDebug() << "[DEMO-AUDIO] result " << result;
...
void AudioRecorder::RegisterEngine(EngineOpenSLES *engine) {
  engine_ = engine;
}
